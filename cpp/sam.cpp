...
struct SamModel {
  ...
  SamModel(const Sam::Parameter& param) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      //option.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_BASIC);
    ...
    if (sessionPre->GetInputCount() != 1 || (int)sessionPre->GetOutputCount() != targetNumber[0]) {
      std::cerr << "Preprocessing model not loaded (invalid input/output count)" << std::endl;
      return;
    }

    sessionSam = std::make_unique<Ort::Session>(env,
                                                wsamModelPath.c_str(), sessionOptions[1]);
    //sessionSam->GetOutputTypeInfo(0);
    //std::cout << "type: " << sessionSam->GetOutputTypeInfo(0).GetMapTypeInfo() << std::endl;
    if ((int)sessionSam->GetInputCount() != targetNumber[1]
            || sessionSam->GetOutputCount() != 3) {
...
//                 .GetTensorTypeAndShapeInfo().GetElementType() << std::endl;
  ...
  bool loadImage(const cv::Mat& image) {
    std::vector<uint8_t> inputTensorValues(inputShapePre[0] * inputShapePre[1]
            * inputShapePre[2] *inputShapePre[3]);
  ...
  void getMask(const std::list<cv::Point>& points,
               const std::list<cv::Point>& negativePoints,
    ...
    inputTensorsSam.push_back(Ort::Value::CreateTensor<float>(
                                  memoryInfo, inputPointValues.data(),
                                  2 * numPoints, inputPointShape.data(),
                                  inputPointShape.size()));
    inputTensorsSam.push_back(Ort::Value::CreateTensor<float>(
                                  memoryInfo, inputLabelValues.data(),
    ...
    inputTensorsSam.push_back(Ort::Value::CreateTensor<float>(
                                  memoryInfo, orig_im_size_values, 2,
...
Sam::Sam(const std::string& preModelPath, const std::string& samModelPath,
         int threadsNumber)
...
cv::Mat Sam::getMask(const std::list<cv::Point>& points,
                     const std::list<cv::Point>& negativePoints,
...
cv::Mat Sam::autoSegment(const cv::Size& numPoints, cbProgress cb,
